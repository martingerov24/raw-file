	...
	int interpolation, int borderType/*, const Scalar& borderValue*/)  // TODO: ASK SASHO FOR SATURATE_CAST
{
	...
	int depth = CV_MAT_DEPTH(16); ////type = src.type(), // after debugging i saw that type was 16, not quite sure though
...
__host__
void undistortPoints(cudaStream_t providedStream, const Matf& points, Matf& points_undistorted, const Matf& K, // K = 3,3
	const Matf& distortion)
{
	Matf k_result = K;
	undistort(points, points_undistorted, K, distortion, 0, k_result); // TODO: invoke directoly the function
}
